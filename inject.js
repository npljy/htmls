(function () {
  ...
  if (loc.includes('leduozy')) {
    ...
    const vodList = _$$('.playlist li') || []
    vodList.forEach(li => {
      const xm = li.innerText
      const nums = xm.split('$')
      const num = nums[0]
      let link = _$$('a', li)[0]?.href
      if (!link) {
        link = nums[1]
      }
      const _a = document.createElement('a')
    ...
    const vodList = _$$('.playlist li') || [];
    const jxUrl = 'https://ffzyplayer.com/?url='
    const titles = _$$('.whitetitle')
    const wraps = _$$('#content h4')
    wraps.forEach(e => {
      if (e.innerText.includes('m3u8')) {
        e.id = 'kk-play-m3u8'
      } else {
    ...
    const index = titles.length - 2
    const titleHtml = titles[index].innerHTML
    titles[index].classList = titles[index].classList + ' flex-block'
    titles[index].innerHTML = '<div class="flex-block center-items">' + titleHtml + '</div>' + '<div><span class="f-s-16">&nbsp;&nbsp;[点击跳转到目标位置]==></span><a class="kk-btn d-i-b" target="_self" href="#kk-play-yun">yun</a><a class="kk-btn d-i-b" target="_self" href="#kk-play-m3u8">m3u8</a></div>'
    vodList.forEach(li => {
      const xm = _$$('font', li)[0].innerText
      const links = xm.split('$')
      const num = links[0]
